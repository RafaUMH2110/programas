...
Uso típico
----------
    python refinar_y_consultar.py

(Los valores de configuración —prompts, modelo, parámetros y ruta de
salida— se definen más abajo, en la sección "CONFIGURACIÓN", o bien pueden
sobrescribirse mediante argumentos de línea de comandos; ver `--help`.)
"""
...
# CONFIGURACIÓN
# ===========================================================================
# Todos los valores de esta sección pueden modificarse directamente aquí,
# o bien sobrescribirse desde la línea de comandos (ver `parse_args()` más
# abajo). Se han agrupado en una única clase de configuración (dataclass)
...
def parse_args(config_por_defecto: ConfiguracionGeneral) -> ConfiguracionGeneral:
    """
    Permite sobrescribir, opcionalmente, algunos valores de configuración
    desde la línea de comandos, sin tener que modificar el código fuente.

    Todos los argumentos son opcionales: si no se indican, se usan los
    valores definidos en `ConfiguracionGeneral` (sección CONFIGURACIÓN, al
    principio del archivo).

    Parameters
    ----------
    config_por_defecto : ConfiguracionGeneral
        Configuración base cuyos valores se usarán si no se pasa el
        argumento correspondiente por línea de comandos.

    Returns
    -------
    ConfiguracionGeneral
        Configuración final, con los valores de línea de comandos aplicados.
    """
    parser = argparse.ArgumentParser(
        description=(
            "Refina un prompt con Claude y luego usa el prompt mejorado "
            "para obtener una respuesta final, guardada en Markdown."
        )
    )
    parser.add_argument(
        "--prompt",
        type=str,
        default=None,
        help="Prompt original del usuario (si no se indica, se usa el definido en el código).",
    )
    parser.add_argument(
        "--salida",
        type=str,
        default=None,
        help="Ruta del archivo Markdown de salida (por ejemplo: ./salida/respuesta.md).",
    )
    parser.add_argument(
        "--modelo",
        type=str,
        default=None,
        help="Modelo de Claude a usar en AMBAS llamadas (paso 1 y paso 2).",
    )
    parser.add_argument(
        "--temperature",
        type=float,
        default=None,
        help="Temperatura a usar en la llamada principal (paso 2). Rango: 0.0 - 1.0.",
    )
    parser.add_argument(
        "--max-tokens",
        type=int,
        default=None,
        help="Máximo de tokens de salida para la llamada principal (paso 2).",
    )

    args = parser.parse_args()

    if args.prompt is not None:
        config_por_defecto.prompt_usuario = args.prompt
    if args.salida is not None:
        config_por_defecto.ruta_salida = Path(args.salida)
    if args.modelo is not None:
        config_por_defecto.config_modelo_refinado.modelo = args.modelo
        config_por_defecto.config_modelo_principal.modelo = args.modelo
    if args.temperature is not None:
        config_por_defecto.config_modelo_principal.temperature = args.temperature
    if args.max_tokens is not None:
        config_por_defecto.config_modelo_principal.max_tokens = args.max_tokens
...
def main() -> None:
    ...
    # 1. Cargar la configuración por defecto y aplicar posibles argumentos
