...
def _guardar(img: Image.Image, nombre: str) -> None:
    img = img.resize((img.width // SS, img.height // SS), Image.LANCZOS) # type: ignore
...
# --------------------------------------------------------------------------
# Esquema JSON "real" (JSON Schema), usado para definir una HERRAMIENTA
# (tool) que el modelo debe rellenar mediante "tool use" / function calling.
#
# Ventaja frente a "pedir JSON como texto libre": la API valida y devuelve
# el argumento de la herramienta ya troceado en `content`, como un objeto
# Python (dict/list) perfectamente formado — no hay que parsear texto ni
# preocuparse por bloques ```json, comillas mal escapadas o respuestas
# cortadas a mitad de una cadena (la causa del error `Unterminated string`
# que se obtiene al pedir JSON como texto normal y quedarse corto de
# `max_tokens`).
# --------------------------------------------------------------------------

_BLOQUE_SCHEMA = {
    "type": "object",
    "description": "Un bloque de contenido dentro de una sección.",
    "properties": {
        "tipo": {
            "type": "string",
            "enum": ["parrafo", "subtitulo", "url_destacada", "vinetas", "nota", "figura"],
        },
        "texto": {"type": "string", "description": "Usado en parrafo, subtitulo, url_destacada y nota."},
        "items": {"type": "array", "items": {"type": "string"}, "description": "Usado en vinetas."},
        "titulo": {"type": "string", "description": "Usado en nota."},
        "estilo": {"type": "string", "enum": ["teal", "orange"], "description": "Usado en nota."},
        "imagen": {"type": "string", "enum": IMAGENES_DISPONIBLES, "description": "Usado en figura."},
        "ancho_cm": {"type": "number", "description": "Usado en figura."},
        "pie": {"type": "string", "description": "Usado en figura."},
    },
    "required": ["tipo"],
}
...
# Esquema completo de entrada de la herramienta que Claude debe rellenar.
HERRAMIENTA_CONTENIDO = {
    "name": "guardar_contenido_guia",
    "description": (
        "Guarda el contenido completo y ya redactado de la guía de acceso a "
        "Google Colab, listo para maquetar en un documento Word."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
            "portada": {
                ...
                "required": [
                    "kicker", "titulo_linea_1", "titulo_linea_2", "subtitulo",
                    "universidad", "grado", "departamento",
                ],
            },
            "indice": {
                "type": "array",
...
def _construir_prompt() -> tuple[str, str]:
    ...
    system = (
        ...
        "universitarios en español de España. Debes llamar SIEMPRE a la "
        "herramienta 'guardar_contenido_guia' con el contenido completo que "
        "se te pide, sin dejar ninguna sección a medias."
    )
...
Llama a la herramienta 'guardar_contenido_guia' con todo este contenido.
...
def obtener_contenido_desde_api() -> dict:
    ...
    y lo devuelve ya convertido en un diccionario Python.

    Usa "tool use" (function calling): en vez de pedirle al modelo que
    escriba JSON como texto libre (frágil: comillas mal escapadas, bloques
    ```json, o respuestas cortadas a mitad de una cadena si se supera
    `max_tokens`), se le obliga a invocar la herramienta
    `guardar_contenido_guia`, cuyos argumentos ya llegan validados contra
    el `input_schema` y perfectamente parseados por el propio SDK.
    """
    ...
    respuesta = cliente.messages.create(
        ...
        # El contenido completo (7 secciones, viñetas, notas, pies de
        # figura…) es largo; con 4000 se llegaba a cortar a mitad de una
        # cadena y `json.loads` fallaba con "Unterminated string". Se sube
        # el límite con margen de sobra.
        max_tokens=8000,
        system=system,
        tools=[HERRAMIENTA_CONTENIDO], # type: ignore
        # Forzamos a que SIEMPRE use esta herramienta (y no responda con
        # texto suelto), para no tener que contemplar ambos casos.
        tool_choice={"type": "tool", "name": "guardar_contenido_guia"},
        messages=[{"role": "user", "content": user}],
    )

    if respuesta.stop_reason == "max_tokens":
        raise RuntimeError(
            "La respuesta de la API se ha cortado por alcanzar el límite de "
            "max_tokens antes de terminar el contenido. Aumenta el valor de "
            "max_tokens en obtener_contenido_desde_api()."
        )

    for bloque in respuesta.content:
        if bloque.type == "tool_use" and bloque.name == "guardar_contenido_guia":
            return bloque.input  # ya es un dict Python, sin parseo manual

    raise RuntimeError(
        "La API no ha devuelto una llamada a la herramienta "
        "'guardar_contenido_guia' como se esperaba."
    )
...
def construir_documento(contenido: dict) -> DocumentObject:
    ...
    estilo_normal.font.name = FONT_NAME # type: ignore
    estilo_normal.font.size = Pt(11) # type: ignore
