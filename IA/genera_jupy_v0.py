...
def responder_ideas(
    ...
    for i, idea in enumerate(ideas, 1):
        ...
        mensaje = client.messages.create(
            ...
            system=system,
            messages=[
                {"role": "user", "content": idea.format(code=code)}
            ],
        )
...
def crear_cliente() -> anthropic.Anthropic:
    ""Crea el cliente de Anthropic. La API key se lee de la variable de entorno ANTHROPIC_API_KEY automáticamente.""
    return anthropic.Anthropic()
...
if __name__ == "__main__":
    ...
    mis_ideas = [
        """Genera un cuaderno de Google Colab en formato JSON a partir de el código python que estará delimitado por
        ```

        Código: ```{code}``` 
        Añade el código necesario para leer la variable de entorno ANTHROPIC_API_KEY y crear el cliente de Anthropic.
        Elimina los delimitadores de código JSON: ```json y ``` al inicio y al final del código generado.
        Elimina cualquier comentario que no sea parte del código y asegúrate de que el cuaderno sea funcional y esté listo para ejecutar.
        Elimina la primera y última línea del cuaderno que contenga delimitadores de código JSON.
        """,
        #"Crea un concepto para una chaqueta de cuero que combine estilo clásico con detalles modernos, y escribe una breve descripción de su diseño.",
    ]

    system_promp = ("""
                    Eres un programador experto en Python notebooks de jupyter
                    Para cada idea, genera un código funcional y una breve descripción.
                    Responde de forma clara y concisa. 
                    """)
    ...
    # Opcional: guardar resultados en un archivo
    print("\n\n📄 Resumen guardado en resultados.ipynb")
    with open("resultados.ipynb", "w", encoding="utf-8") as f:
        for r in resultados:
            # f.write(f"IDEA: {r['idea']}\n")
            # f.write(f"RESPUESTA:\n{r['respuesta']}\n")
            # f.write("-" * 60 + "\n")
