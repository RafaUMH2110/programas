...
Módulo compartido utilizado por ``generate_presentation_vscode.py`` y
...
Dependencias (instalar con pip):
    ...
    pip install python-pptx python-docx anthropic
...
def nueva_diapositiva(prs: Presentation, fondo: str) -> Any:
...
def agregar_texto(
    ...
    for indice, parrafo in enumerate(parrafos):
        ...
        for fragmento in parrafo:
            ...
            run.font.size = Pt(fragmento.get("tamano", tamano))
...
def extraer_contenido_docx(ruta_docx: Path) -> str:
    ...
    for parrafo in documento.paragraphs:
        ...
        if texto:
            ...
            if estilo.lower().startswith("heading") or estilo.lower().startswith("título"):
                bloques.append(f"\n### {texto}\n")
            else:
...
# ---------------------------------------------------------------------------
# 5. CONSTRUCCIÓN DEL PROMPT Y LLAMADA AL LLM (API DE ANTHROPIC / CLAUDE)
# ---------------------------------------------------------------------------
...
class ConfiguracionLLM:
    """Parámetros de la llamada al modelo de lenguaje (Claude, vía la API
    de Anthropic).

    El nombre del modelo puede cambiar con el tiempo a medida que
    Anthropic publica nuevas versiones; algunos valores válidos en el
    momento de escribir este código son: ``"claude-sonnet-5"``
    (equilibrio entre calidad y coste, recomendado por defecto),
    ``"claude-opus-4-8"`` (máxima calidad para contenido muy complejo)
    y ``"claude-haiku-4-5-20251001"`` (más rápido y económico).
    """
...
def modelo_admite_temperatura(modelo: str) -> bool:
    """Indica si el modelo admite el parámetro ``temperature``."""
    return not any(modelo.startswith(prefijo) for prefijo in MODELOS_SIN_TEMPERATURA)


def construir_prompt_llm(contenido_docx: str) -> tuple[str, str]:
    """Construye el prompt de sistema y el prompt de usuario para Claude.

    A diferencia de otras APIs de chat, la API de Anthropic separa el
    mensaje de sistema (parámetro ``system``) de la lista de mensajes
    (``messages``), por lo que esta función devuelve ambas partes por
    separado en lugar de una única lista combinada.
    """
    sistema = (
...
def generar_estructura_presentacion(
    ...
    """Llama a la API de Anthropic (Claude) y devuelve el diccionario JSON
    ya parseado.

    Incluye una lógica de reintentos ante fallos de red, respuestas JSON
    malformadas y, además, ante dos incompatibilidades conocidas y
    dependientes del modelo concreto que se use:

    * Algunos modelos ya no admiten el parámetro ``temperature``.
    * Algunos modelos no admiten "prefill" del mensaje del asistente
      (forzar que la respuesta empiece con un texto que nosotros le
      proporcionamos, en este caso ``"{"``, para garantizar una salida
      JSON limpia).

    La incompatibilidad de ``temperature`` se comprueba antes de la
    primera llamada para los modelos conocidos y se mantiene un reintento
    de respaldo si la API informa de una variante aún no registrada. La
    incompatibilidad de prefill se detecta a partir del error 400 y el
    proceso se adapta sobre la marcha, sin gastar un intento de los
    disponibles para fallos genuinos.
    """
    cliente = Anthropic()  # Lee ANTHROPIC_API_KEY automáticamente desde el entorno
    sistema, usuario = construir_prompt_llm(contenido_docx)

    incluir_temperatura = modelo_admite_temperatura(config.modelo)
    usar_prefill = True
    ...
    while intento <= config.intentos_maximos:
        try:
            ...
            respuesta = cliente.messages.create(**parametros_llamada)

            fragmentos = [bloque.text for bloque in respuesta.content if bloque.type == "text"]
            texto_bruto = "".join(fragmentos)
            if usar_prefill:
                # El "{" del prefill no viene incluido en la respuesta.
            ...
            if incluir_temperatura and "temperature" in mensaje_error and "deprecat" in mensaje_error:
                logger.warning(
                    "El modelo '%s' no admite el parámetro 'temperature' "
                    "(obsoleto para este modelo); se reintenta sin él.",
                    config.modelo,
                )
                incluir_temperatura = False
                continue  # No se cuenta como fallo: no se incrementa "intento"
            if usar_prefill and "prefill" in mensaje_error:
                logger.warning(
                    "El modelo '%s' no admite el prefill del mensaje del "
                    "asistente; se reintenta pidiendo el JSON de forma "
                    "explícita en el mensaje del usuario.",
                    config.modelo,
                )
                usar_prefill = False
                continue  # No se cuenta como fallo: no se incrementa "intento"
...
def render_portada(prs: Presentation, meta: dict, numero: int) -> None:
...
def render_section(prs: Presentation, s: dict, numero: int) -> None:
...
def render_bullets_two_col(prs: Presentation, s: dict, numero: int) -> None:
...
def render_process_flow(prs: Presentation, s: dict, numero: int) -> None:
...
def render_table(prs: Presentation, s: dict, numero: int) -> None:
...
def render_cards(prs: Presentation, s: dict, numero: int) -> None:
...
def render_grafcet_diagram(prs: Presentation, s: dict, numero: int) -> None:
...
def render_closing(prs: Presentation, s: dict, numero: int) -> None:
...
def generar_presentacion_desde_docx(
    ...
    -------
    Path
        Ruta final del archivo .pptx generado.
    """
    ...
    datos = generar_estructura_presentacion(contenido, config_llm)
